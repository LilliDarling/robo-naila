...
class DeviceConfig:
    ...
    def from_env_and_args(cls) -> DeviceConfig:
        ...
        def _resolve(env_key: str, arg_val, default):
            if arg_val is not None:
                return arg_val
            env = os.environ.get(env_key)
            if env is not None:
                return type(default)(env) if not isinstance(default, type(None)) else env
            return default
        ...
        return cls(
            ...
            sample_rate=_resolve("NAILA_SAMPLE_RATE", args.sample_rate, cls.sample_rate),
            frame_duration_ms=_resolve("NAILA_FRAME_DURATION_MS", args.frame_duration_ms, cls.frame_duration_ms),
            channels=_resolve("NAILA_CHANNELS", args.channels, cls.channels),
            reconnect_delay=_resolve("NAILA_RECONNECT_DELAY", args.reconnect_delay, cls.reconnect_delay),
            max_reconnect_delay=_resolve("NAILA_MAX_RECONNECT_DELAY", args.max_reconnect_delay, cls.max_reconnect_delay),
