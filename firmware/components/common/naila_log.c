...
static void stats_monitoring_task(void *parameters) {
  while (!stats_task_should_stop) {
    ...
    if (g_stats.uptime_sec % 600 == 0 && g_stats.uptime_sec > 0) {
      NAILA_LOGI(STATS_TAG,
          "System status - Uptime: %lu sec, Free heap: %zu bytes",
...
naila_err_t naila_stats_start_task(void) {
  ...
  BaseType_t result = xTaskCreate(
      stats_monitoring_task, "stats_mon", 3072, NULL, 2, &stats_task_handle);
...
bool naila_stats_is_task_running(void) { return stats_task_handle != NULL; }