...
class ConversationMemory:
    ...
    def _start_background_cleanup(self):
        ...
        try:
            ...
            logger.info("No event loop found - background cleanup disabled")
    ...
    def add_exchange(self, device_id: str, user_msg: str, assistant_msg: str, metadata: Dict = None):
        ...
        logger.debug(f"Added exchange for {device_id}, history size: {len(self.conversations[device_id])}")
    ...
    def get_history(self, device_id: str, limit: int = None) -> List[Dict]:
        ...
        if limit and limit < len(conversation):
            # Convert deque slice to list efficiently
    ...
    def clear_device(self, device_id: str):
        ...
        if device_id in self.conversations:
            ...
            logger.info(f"Cleared conversation history for {device_id}")
    ...
    def cleanup_old_conversations(self):
        ...
        with self._lock:
            ...
            for device_id in list(self._active_devices):
                ...
                if original_len != len(conversation):
                    logger.debug(f"Cleaned {device_id}: {original_len} -> {len(conversation)} exchanges")
        ...
        if removed_devices:
            logger.info(f"Cleaned up {len(removed_devices)} old conversations, {len(self.conversations)} devices remain")
